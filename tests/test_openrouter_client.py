...
from analytics_workflow.runtime_config import DEFAULT_MODEL
...
class OpenRouterContentExtractionTests(unittest.TestCase):
    ...
    def test_extracts_openrouter_reasoning_fields(self) -> None:
        data = {
            "choices": [
                {
                    "message": {
        ...
        reasoning = OpenRouterClient._extract_message_reasoning(data)
        self.assertEqual(reasoning["reasoning"], "thinking text")
        self.assertEqual(reasoning["reasoning_details"], [{"type": "reasoning.text", "text": "detail"}])
        self.assertEqual(reasoning["reasoning_content"], "deepseek thinking")


class OpenRouterRetryBehaviorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.client = OpenRouterClient("k", DEFAULT_MODEL)

    def test_deepseek_v32_requests_reasoning_from_openrouter(self) -> None:
        ...
        with patch.object(self.client.session, "post", side_effect=fake_post), patch(
            ...
            result = self.client.chat_completion("sys", "user", max_retries=1)
    ...
    def test_non_deepseek_v32_does_not_add_reasoning_payload(self) -> None:
        ...
        with patch.object(client.session, "post", side_effect=fake_post), patch(
            "analytics_workflow.clients.time.sleep"
        ):
                ...
                "choices": [
                    {
                        "message": {
                            "content": "ok",
                            "reasoning": "brief thinking",
                            "reasoning_details": [{"type": "reasoning.text", "text": "brief thinking"}],
                        }
                    }
                ]
            }
        )
        with patch.object(self.client.session, "post", return_value=response), patch(
            "analytics_workflow.clients.time.sleep"
        ):
            self.client.chat_completion("sys", "user", max_retries=1)

        self.assertEqual(self.client.last_reasoning["reasoning"], "brief thinking")
        self.assertEqual(len(self.client.reasoning_history), 1)
    ...
    def test_chat_completion_caps_max_tokens_after_credit_limit(self) -> None:
        limited = _response(
            {"error": {"message": "This request requires more credits, or fewer max_tokens. You requested up to 4000 tokens, but can only afford 1606."}},
            status_code=402,
        )
        good = _response({"choices": [{"message": {"content": "ok"}}]})
        captured_max_tokens: list[int] = []
        ...
        def fake_post(url, json=None, timeout=None):
            captured_max_tokens.append(json["max_tokens"])
            return [limited, good][len(captured_max_tokens) - 1]
